...
void testTL1XvsY()
{
    ...
    while( event->Next() )
    {
        ...
        if( mhtPassFlag && recalcRecoMht > 0.0 && l1Mht > 0.0 )
            xvsy[1]->Fill(recalcRecoMht, l1Mht);
        ...
        if( mhtPassFlag && recalcRecoMht > 0.0 && recalcL1Mht > 0.0 )
        {
            ...
            xvsy[5]->Fill(FoldPhi(recalcRecoMhtPhi), FoldPhi(l1MhtPhi));
            xvsy[6]->Fill(recalcRecoMht, recalcL1Mht);
            xvsy[7]->Fill(event->fNJetRecoMht, event->fNJetL1Mht);
        }

        if( recoMht > 0.0 && l1Mht > 0.0 )
            xvsy[8]->Fill(recoMht, l1Mht);
